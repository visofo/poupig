...
import OrdenarPorValor from '../../src/extrato/filter/OrdenarPorValor'
...
test('Deve ordenar por valor', () => {
    ...
    const novasTransacoes = OrdenarPorValor.aplicar()(transacoes)
    expect(novasTransacoes).toHaveLength(4)
    expect(novasTransacoes[0]!.valor).toBe(500)
    expect(novasTransacoes[1]!.valor).toBe(600)
    expect(novasTransacoes[2]!.valor).toBe(800)
    expect(novasTransacoes[3]!.valor).toBe(1000)
})
